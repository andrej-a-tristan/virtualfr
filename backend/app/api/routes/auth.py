...
from app.api.store import set_session_user, clear_session
...
def login(body: LoginRequest, response: Response):
    """Login: Supabase Auth when configured, else mock session."""
    if is_supabase_configured():
        try:
            client = get_supabase()
        except Exception as e:
    ...
    _set_session_cookie(response, SESSION_VALUE)
    return _user_response(user_id, body.email, None, False, False, None)
