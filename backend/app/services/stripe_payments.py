...
def ensure_customer(sid: str, user: dict) -> tuple[str, dict]:
    ...
    # Sync billing identifiers from Supabase when available so all payment
    # entry points reuse the same saved card/customer.
    try:
...
def resolve_default_pm(sid: str, user: dict, customer_id: str) -> tuple[str | None, dict]:
    ...
    try:
        ...
        inv_settings = cust.get("invoice_settings") if isinstance(cust, dict) else None
        if inv_settings:
            pm = inv_settings.get("default_payment_method")
            if pm:
